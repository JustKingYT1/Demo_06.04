...
import settings
from src.server.database.database_models import User
...
class MenuTableViews(QtWidgets.QFrame):
    ...
    def include_widgets_for_pl(self) -> None:
        include_widgets(User.get(User.id == self.parent.session.user.id).position.power_level, self.scroll_layout)
    ...
    def add_button_clicked(self) -> None:
        dialog = TableAction(self, self.router, 'Добавить в')
    ...
    def upd_button_clicked(self) -> None:
        ...
        dialog = TableAction(self, self.router, 'Изменить в')

        dialog.setWindowTitle(f'Изменить {self.router.object_name}')
    ...
    class ModifyButton(QtWidgets.QPushButton):
        ...
        def set_router(self, router):
            self.router = router
            self.setProperty('power_level', router.power_level)
            print(self.property('power_level'))